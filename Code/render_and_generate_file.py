...
class molecule:
    ...
    def get_colour(self, index):
        """
        Gets colour of atom within the molecule for graph
        """
        #Compares symbol of atom at specified index and compares to atoms
...
def file_name_gen(mol_type): 
    ...
    """
    name = (mol_type + '_' + str(date.today()) + '_' + str(datetime.now().strftime("%H-%M-%S"))+ ".txt")
    return name

def build_orca_file(mol_type, mol_comp, mol_co_ords): 
    """
    Fucntion generates the Orca input file
    """
    #Orca function is state and stored in variable
    orca_function = "! B3LYP def2-SVP SP"	
    #Create filename for Orca input file
    work_file_name = file_name_gen(mol_type)
    #Creates new directory and the text file using the filename generated
    os.mkdir(work_file_name)
    file_script = open(work_file_name+ "\\"+ str(work_file_name),"a")
    ...
    return work_file_name #Returns filename

def run_orca(file_name):
    """
    ...
    os.chdir(current_dir + "\\"+ file_name)
    #Wriets command line
